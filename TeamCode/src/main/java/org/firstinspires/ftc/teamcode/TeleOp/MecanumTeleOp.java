...
public class MecanumTeleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        sleep(1750); // Allow the motor to retract for 0.5 seconds (adjust time as needed)
    ...
    public void lift() {
        if (gamepad1.dpad_up && liftTargetPosition < MAX_LIFT_TICKS) {
            liftTargetPosition += (int) (.3 * TICKS_PER_INCH); // Increase position by 1 inch
            robot.intakeRotation.setPosition(.4);

        } else if (gamepad1.dpad_down && liftTargetPosition > MIN_LIFT_TICKS) {
            liftTargetPosition -= (int) (1 * TICKS_PER_INCH); // Decrease position by 1 inch
        }
        else if (gamepad1.left_bumper) {
            robot.lift.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER); // Use encoder mode for teleop
